...
public class YOLOv10Detector {
    ...
    public List<Detection> detect(Bitmap bitmap, int imageRotation) {
        try {
            ...
            if (imageRotation != 0) {
                ImageProcessor rotationProcessor = new ImageProcessor.Builder()
                    .add(new Rot90Op(-imageRotation / 90))
                    .add(new ResizeOp(INPUT_SIZE, INPUT_SIZE, ResizeOp.ResizeMethod.BILINEAR))
                    .build();
                tensorImage = rotationProcessor.process(tensorImage);
            } else {
    ...
    private List<Detection> postProcess(float[][] outputs, int originalWidth, int originalHeight) {
        ...
        for (int i = 0; i < outputs.length; i++) {
            ...
            if (output.length < 6) continue;
            
            // Extract detection data - format appears to be [x1, y1, x2, y2, confidence, class_id]
            float x1 = output[0];
            float y1 = output[1]; 
            float x2 = output[2];
            float y2 = output[3];
            ...
            if (i < 5 && confidence > 0.1f) {
                Log.d(TAG, "Detection " + i + ": bbox=[" + x1 + "," + y1 + "," + x2 + "," + y2 + "], conf=" + confidence + ", class=" + (int)classId);
            }
            
            // Filter by confidence threshold
            if (confidence < CONFIDENCE_THRESHOLD) continue;
            
            // YOLOv10 coordinates are normalized (0-1), so we need to convert them directly to pixel coordinates
            // without additional scaling since they are already relative to the original image size
            ...
            Log.d(TAG, "Converted coordinates: left=" + left + ", top=" + top + ", right=" + right + ", bottom=" + bottom);
            
            // Get class name
            int classIdInt = (int) classId;
            String className = (classIdInt >= 0 && classIdInt < CLASS_NAMES.length) 
                ? CLASS_NAMES[classIdInt] : "unknown";
        ...
        // Since this model appears to have NMS already applied, we may not need additional NMS
